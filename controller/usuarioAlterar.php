<?php
    ...
    if($nome != "" && $id != "" && $autenticado===true){
        try{
            ...
            $stmt->bindParam(2, $id);
            $stmt->execute();
            $resposta["msgSucesso"] = "{$stmt->rowCount()} usuario alterado com sucesso! O is do usuario alterado foi {$id}";
        }catch(PDOException $e){
            $resposta["erro"] = true;
            $resposta["msgErro"] = "Erro: Não foi possivel efetuar a alteracao no BD".$e->getMessage();
        }finally{
